...
def render_ul_assignment_dashboard(df: pd.DataFrame):
    ...
    channels = sorted(df["channle"].unique())
    selected_ch = st.sidebar.multiselect(
        "Select Channel", options=channels, default=channels, key="selected_channel"
    )
    # 根據篩選過濾資料
    filtered_df = df[df['channle'].isin(selected_ch)]
