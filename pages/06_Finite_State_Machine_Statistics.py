...
from modules.GCT_Finite_State_Machine import render_toa_statistics
...
def fragment_toa_statistics():
    global_data = get_global_data()
    df_raw = pd.DataFrame(global_data.get_toa_sample())
    if render_if_changed(df_raw,"last_toa_sample"):
        render_toa_statistics(df_raw)
    else:
        print("Did not need update page 04_Finite_State_Machine_Statistics")
